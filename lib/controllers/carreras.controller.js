...
/**
 * Entrada: Id carrera e Id de plan
 * Salida: Carrera con Id y plan Id correspondiente
 */
export async function obtenerCarreraConPlan(req, res) {
  try {
    ...
    if (req.query.careerId) {
      filtro.careerId = req.query.careerId;
      (filtro.planId = req), query.planId;
    }
    const searchData = await carrerModel.find(filtro); //Devuelve un array .find()
    const careerData = searchData[0];
    //res.json({careerData})
