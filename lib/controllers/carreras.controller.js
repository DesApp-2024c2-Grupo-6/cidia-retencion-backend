import mongoose from 'mongoose';
import { CareerDataSchema } from '../models/carreras.schema';
import { validarCampo } from './utils.controller';
import {
  getAllCarreras,
  getAllCarrerasConPlanes,
} from '../services/carrerasService';

//por el contexto de la app es posible que este metodo no lo use desde el FE.
export async function crearCarrera(req, res) {
  try {
    const carrerModel = mongoose.model('Carrera', CareerDataSchema);
    const {
      careerId,
      planId,
      unahurSubjects,
      englishLevels,
      suggestionThresholdRegularizedSubjects,
      specialCareerName,
      minimumSubjectsRecommended,
    } = req.body;

    const nuevaCarrera = new carrerModel({
      careerId: validarCampo(careerId),
      planId: validarCampo(planId),
      unahurSubjects,
      englishLevels,
      suggestionThresholdRegularizedSubjects: validarCampo(
        suggestionThresholdRegularizedSubjects
      ),
      specialCareerName: validarCampo(specialCareerName),
      minimumSubjectsRecommended: validarCampo(minimumSubjectsRecommended),
    });
    await nuevaCarrera.save();
    res.status(200).json(nuevaCarrera);
  } catch (error) {
    res.status(500).json({ error });
  }
}
/**
 * Obtener datos de 1 Carrera.
 * @param {careerId} req id de carrera
 * @param {*} res devuelve un objeto con el dato de la carrera encontrado sino vacío.
 */
export async function obtenerCarrera(req, res) {
  try {
    const carrerModel = mongoose.model('Carrera', CareerDataSchema);
    //en filtro agrego el objeto por el cual filtrar tiene q ser el mismo campo del documento
    const filtro = {};
    if (req.query.careerId) {
      filtro.careerId = req.query.careerId;
    }
    const searchData = await carrerModel.find(filtro); //Devuelve un array .find()
    const careerData = searchData[0];
    //res.json({careerData})
    if (careerData) {
      res.status(200).json({ careerData });
    } else {
      res.status(204).send();
    }
  } catch (error) {
    res.status(500).json({ error });
  }
}

/**
 *
 * @param {*} req Datos de carrera a cambiar
 * @param {*} res Datos de carrera actualizada o error
 */
export async function actualizarCarrera(req, res) {
  try {
    const carrerModel = mongoose.model('Carrera', CareerDataSchema);
    const {
      careerId,
      unahurSubjects,
      englishLevels,
      suggestionThresholdRegularizedSubjects,
      specialCareerName,
      minimumSubjectsRecommended,
      planId,
    } = req.body;
    const filtro = { careerId: careerId };
    const objCarrera = {
      careerId: Number(careerId),
      unahurSubjects,
      englishLevels,
      suggestionThresholdRegularizedSubjects:
        suggestionThresholdRegularizedSubjects !== undefined
          ? suggestionThresholdRegularizedSubjects
          : undefined,
      specialCareerName,
      minimumSubjectsRecommended:
        minimumSubjectsRecommended !== undefined
          ? minimumSubjectsRecommended
          : undefined,
      planId,
    };
    const updateCareer = await carrerModel.findOneAndUpdate(filtro, objCarrera);
    console.log(updateCareer);
    if (updateCareer) {
      res.status(200).json({ updateCareer });
    } else {
      res.status(204).send();
    }
  } catch (error) {
    res.status(500).json({ error });
  }
}

export async function obtenerTodasLasCarreras(req, res) {
  try {
    const carrerModel = mongoose.model('Carrera', CareerDataSchema);
    const allCareers = await carrerModel.find();
    const carrerasMock = await getAllCarreras();
    const carrerasCompletas = allCareers.map((carrera) => {
      if (carrera.careerId != undefined) {
        const carreraMock = carrerasMock.data.find(
          (c) => c.id === carrera.careerId
        );
        carrera.careerName = carreraMock.nombre;
      }
      return carrera;
    });
    if (carrerasCompletas) {
      res.status(200).json({ allCareers: carrerasCompletas });
    } else {
      res.status(204).send();
    }
  } catch (error) {
    res.status(500).json({ error });
  }
}

/**
 * Entrada: Ninguna
 * Salida: Todas las carreras con los planes de estudio incluido
 */
export async function obtenerTodasLasCarrerasConPlanes(req, res) {
  try {
    const carrerModel = mongoose.model('Carrera', CareerDataSchema);
    const allCareers = await carrerModel.find();
    const carrerasMock = await getAllCarrerasConPlanes();
    const carrerasCompletas = allCareers.map((carrera) => {
      if (carrera.careerId != undefined) {
        const carreraMock = carrerasMock.data.find(
          (c) => c.id === carrera.careerId
        );
        const datosPlan = carreraMock.planes.find(
          (plan) => plan.id === carrera.planId
        );
        carrera.careerName = carreraMock.nombre;
        carrera.planName = isNaN(datosPlan.nombre.slice(-4))
          ? `${datosPlan.nombre} - ${datosPlan.fecha_entrada_vigencia.slice(
              0,
              4
            )}`
          : datosPlan.nombre;
      }
      return carrera;
    });
    if (carrerasCompletas) {
      res.status(200).json({ allCareers: carrerasCompletas });
    } else {
      res.status(204).send();
    }
  } catch (error) {
    res.status(500).json({ error });
  }
}

export async function eliminarCarrera(req, res) {
  try {
    const carrerModel = mongoose.model('Carrera', CareerDataSchema);
    const filtro = { careerId: Number(req.query.careerId) };
    const eliminadaCarrera = await carrerModel.findOneAndDelete(filtro);
    if (eliminadaCarrera) {
      res.status(200).json({ eliminadaCarrera });
    } else {
      res.status(204).send();
    }
  } catch (error) {
    res.status(500).json({ error });
  }
}

/**
 * Entrada: Ninguna
 * Salida: Carreras de guarani con formato de schema
 */
export async function obtenerTodasLasCarrerasGuarani(req, res) {
  try {
    const carrerasMock = await getAllCarreras();
    res.status(200).json(carrerasMock.data);
  } catch (error) {
    console.log(error);
    res.status(500).json({ error });
  }
}

export async function obtenerTodasLasCarrerasGuaraniConPlanes(req, res) {
  try {
    const carrerasMock = await getAllCarrerasConPlanes();
    res.status(200).json(carrerasMock.data);
  } catch (error) {
    console.log(error);
    res.status(500).json({ error });
  }
}

/**
 * Entrada: Id de carrera e id de plan
 * Salida: Carrera que tenga un id de carrera valido y un id de plan correspondiente a la carrera
 */
export async function obtenerCarreraConPlan(req, res) {
  try {
    const carrerModel = mongoose.model('Carrera', CareerDataSchema);
    const careerId = req.params.id;
    const planId = req.params.planId;
    const carrera = await carrerModel.findOne({
      careerId: careerId,
      planId: planId,
    });
    if (carrera) {
      res.status(200).json({ carrera });
    } else {
      res.status(204).send();
    }
  } catch (error) {
    res.status(500).json({ error });
  }
}
