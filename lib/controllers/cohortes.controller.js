...
async function alumnosRegulzarizadosExactamenteEnMaterias(
  idPeriodo,
  cantidadDeseada
) {
  const SubjectStatus = mongoose.model('SubjectStatus', subjectStatusSchema);
...
async function cohorteDePeriodo(
  ...
  cohorteData.terminaron = alumnosGuaraniPeriodoActual.filter(
    (a) => a.egresado
  ).length;

  // Prepara tareas en paralelo para inscriptos y regularizaron
  const inscriptosPromesas = [1, 2, 3, 4].map((n) =>
    alumnosAnotadosAExactamenteMaterias(idPeriodo, n)
  );
  const regularizaronPromesas = [1, 2, 3, 4].map((n) =>
    alumnosRegulzarizadosExactamenteEnMaterias(idPeriodo, n)
  );
  const [
    exactamenteUna,
    exactamenteDos,
    exactamenteTres,
    cuatroOMas,
  ] = await Promise.all(inscriptosPromesas);
  const [
    regExactamenteUna,
    regExactamenteDos,
    regExactamenteTres,
    regCuatroOMas,
  ] = await Promise.all(regularizaronPromesas);
