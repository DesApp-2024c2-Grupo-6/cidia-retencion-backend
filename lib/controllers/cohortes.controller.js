...
async function obtenerDatosDeAlumnoEnPeriodo(
  Schema,
  alumno,
  idPeriodo,
  ultimosCuatrimestres
) {
  ...
  const query = await Schema.aggregate([
        ...
        cantidadRegAcumuladas: {
          $size: {
            $filter: {
              ...
              cond: {
                $and: [
                  { $in: [{ $type: '$$materia.periodoReg' }, ['int', 'long', 'double']] },
                  { $lte: ['$$materia.periodoReg', idPeriodo] }
                ]
              }
            },
          },
    ...
    },
    {
      $addFields: {
        abandono: {
          $and: [{ $eq: ['$sigue', false] }, { $eq: [alumno.egresado, false] }],
        },
        termino: {
          $and: [{ $eq: ['$sigue', false] }, { $eq: [alumno.egresado, true] }],
        },
      },
    },
  ]);
  ...
  const alumnoData = {
    ...
    cantidadRegularizadasAcumuladas: resultado?.cantidadRegAcumuladas || 0,
...
function agregarDatosDeAlumnoACohorte(cohorteData, alumnoData) {
  ...
  cohorteData.inscriptos.exactamenteUna += alumnoData.inscripto.exactamenteUna
    ? 1
    : 0;
  cohorteData.inscriptos.exactamenteDos += alumnoData.inscripto.exactamenteDos
    ? 1
    : 0;
  cohorteData.inscriptos.exactamenteTres += alumnoData.inscripto.exactamenteTres
    ? 1
    : 0;
  cohorteData.inscriptos.unaMateria += alumnoData.inscripto.unaMateria ? 1 : 0;
  cohorteData.inscriptos.dosMaterias += alumnoData.inscripto.dosMaterias
    ? 1
    : 0;
  cohorteData.inscriptos.tresMaterias += alumnoData.inscripto.tresMaterias
    ? 1
    : 0;
  ...
  cohorteData.regularizaron.exactamenteUna += alumnoData.regularizo
    .exactamenteUna
    ? 1
    : 0;
  cohorteData.regularizaron.exactamenteDos += alumnoData.regularizo
    .exactamenteDos
    ? 1
    : 0;
  cohorteData.regularizaron.exactamenteTres += alumnoData.regularizo
    .exactamenteTres
    ? 1
    : 0;
  cohorteData.regularizaron.unaMateria += alumnoData.regularizo.unaMateria
    ? 1
    : 0;
  cohorteData.regularizaron.dosMaterias += alumnoData.regularizo.dosMaterias
    ? 1
    : 0;
  cohorteData.regularizaron.tresMaterias += alumnoData.regularizo.tresMaterias
    ? 1
    : 0;
  cohorteData.regularizaron.cuatroOMas += alumnoData.regularizo.cuatroOMas
    ? 1
    : 0;

  //Regularizados:
  cohorteData.regularizados.una +=
    alumnoData.cantidadRegularizadasAcumuladas == 1 ? 1 : 0;
  cohorteData.regularizados.dos +=
    alumnoData.cantidadRegularizadasAcumuladas == 2 ? 1 : 0;
  cohorteData.regularizados.tresACinco +=
    alumnoData.cantidadRegularizadasAcumuladas >= 3 &&
      alumnoData.cantidadRegularizadasAcumuladas <= 5
      ? 1
      : 0;
  cohorteData.regularizados.seisADiez +=
    alumnoData.cantidadRegularizadasAcumuladas >= 6 &&
      alumnoData.cantidadRegularizadasAcumuladas <= 10
      ? 1
      : 0;
  cohorteData.regularizados.onceAQuince +=
    alumnoData.cantidadRegularizadasAcumuladas >= 11 &&
      alumnoData.cantidadRegularizadasAcumuladas <= 15
      ? 1
      : 0;
  cohorteData.regularizados.dieciseisAVeinte +=
    alumnoData.cantidadRegularizadasAcumuladas >= 16 &&
      alumnoData.cantidadRegularizadasAcumuladas <= 20
      ? 1
      : 0;
  cohorteData.regularizados.veintiunoOMas +=
    alumnoData.cantidadRegularizadasAcumuladas >= 21 ? 1 : 0;
...
async function cantidadRegularizadosEnMateria(
  Schema,
  idsAlumnos,
  idPeriodo,
  idMateria
...
async function cohorteDePeriodo(
  alumnosGuaraniData,
  idPeriodo,
  materiasDeCarrera
) {
  ...
  await Promise.all(
    alumnosGuaraniData.map(async (alumno) => {
      const alumnoData = await obtenerDatosDeAlumnoEnPeriodo(
        SubjectStatus,
        alumno,
        idPeriodo,
        ultimosCuatrimestres
  ...
  cohorte.materias = await Promise.all(
    materiasDeCarrera.map(async (materia) => ({
      ...
      regularizaron: await cantidadRegularizadosEnMateria(
        SubjectStatus,
        idsAlumnosGuarani,
        idPeriodo,
        materia.id
...
async function cuatrimestresSiguientes(idPeriodo) {
  ...
  const siguientes = idCuatrimestresOrdenados.slice(
    indice,
    idCuatrimestresOrdenados.length - 1
  );
  return siguientes;
}
...
const alumnosDeCohorte = async (idCarrera, idPeriodo) => {
  ...
  const alumnosDePeriodo = alumnosGuarani.filter(
...
const materiasConAnio = async (idCarrera) => {
  const Materia = mongoose.model('subjectSchame', subjectsSchema);
<<<<<<< HEAD
  const materias = await Materia.find({ id_carrera: idCarrera })
  const materiasGuarani = (await getSubj(idCarrera)).data.filter((materia) => !materia.esUnahur);
=======
  const materias = await Materia.find({ id_carrera: idCarrera });
  const materiasGuarani = (await getSubj(idCarrera)).data.filter(
    (materia) => !materia.esUnahur
  );
>>>>>>> 32bb52404b9ab501dbd05071f98c6f3299db40a5
  const materiasConAnio = materias.map((materia) => {
    if (materia.id_materia != undefined) {
      const materiaGuarani = materiasGuarani.find(
        (materiaGuarani) => materiaGuarani['id'] === materia['id_materia']
      );
      if (materiaGuarani != undefined) {
<<<<<<< HEAD
        materia.subjectName = `${(materia?.anio || "?")} año - ${materia?.specialSubjectName || materiaGuarani.nombre}`;
=======
        materia.subjectName = `${materia?.anio || '?'} año - ${
          materia?.specialSubjectName || materiaGuarani.nombre
        }`;
>>>>>>> 32bb52404b9ab501dbd05071f98c6f3299db40a5
      }
    }
    return { nombre: materia.subjectName, id: materia.id_materia };
  });
<<<<<<< HEAD

  return materiasConAnio;
=======
>>>>>>> 32bb52404b9ab501dbd05071f98c6f3299db40a5
...
export async function obtenerCohorteCarrera(req, res) {
  try {
    ...
    const materiasSinOrdenar = await materiasConAnio(idCarrera);

    const materiasDeCarrera = materiasSinOrdenar.sort((a, b) =>
    ...
    const alumnosGuaraniData = (
      await alumnosDeCohorte(idCarrera, idPeriodo)
    ).map((alumno) => ({ id: alumno.alumno, egresado: alumno.egresado }));

    //const alumnosGuaraniData = [{ id: 53857, egresado: true }, { id: 58108, egresado: false }, { id: 50946, egresado: true }]
          ...
          },
          {
            $group: {
              ...
              inscriptos_4: {
                $sum: {
                  $cond: [
                    { $eq: ['$status_materias.cantidadCursadas', 4] },
                    1,
                    0,
                  ],
                },
              ...
              intentosCursada: {
                $sum: '$status_materias.cantidadCursadas',
              },
            },
          },
        ],
                  ...
                  $cond: [
                    {
                      $and: [
                        {
                          $eq: [
                            '$status_materias.periodoUltimaCursada',
                            idPeriodo,
                          ],
                        },
                        { $eq: ['$status_materias.cantidadCursadas', 1] },
